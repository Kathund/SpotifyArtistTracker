  ...
  override async handle(req: Request, res: Response) {
    try {
      ...
      if (result.status === 403 || result.status === 401) {
        return res
          .status(403)
          .json({ success: true, cause: "There isn't a Spotify account that is logged in. Please contact the owner" });
      }
      ...
      res.status(200).json({ success: true, message: 'Token generated successfully' });
    } catch (error) {
